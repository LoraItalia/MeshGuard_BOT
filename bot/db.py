...
@contextmanager
def get_db():
    """Context manager that yields an sqlite3 connection with row factory."""
    ...
    with get_db() as conn:
        ...
        c.execute(
            ...
            CREATE TABLE IF NOT EXISTS nodes (
                node_num    TEXT PRIMARY KEY,  -- id hex senza '!'
                short_name  TEXT,
                long_name   TEXT,
...
def upsert_node(conn, node_num: str,
    ...
    now = utc_now_iso()
    cur = conn.execute(
        "SELECT node_num FROM nodes WHERE node_num = ?",
        (node_num,),
    )
    row = cur.fetchone()
    if row:
...
def create_link_token(chat_id: int) -> str:
    ...
    token = secrets.token_hex(4).upper()   # 8 caratteri hex
    with get_db() as conn:
        now = utc_now_iso()
        conn.execute(
...
def consume_link_token(conn, token: str):
    """Ritorna chat_id se il token esiste ed è libero, altrimenti None."""
    cur = conn.execute(
...
def get_nodes_for_chat(chat_id: int) -> List[sqlite3.Row]:
    with get_db() as conn:
        cur = conn.execute(
            """
            SELECT m.node_num,
                   COALESCE(m.local_name, n.short_name, n.long_name, m.node_num) AS display_name,
                   n.short_name,
                   n.long_name,
                   n.loraitalia_id,
                   m.verified_at
...
def get_node_display_name(conn, node_num: str) -> str:
    cur = conn.execute(
        """
        SELECT COALESCE(short_name, long_name, node_num) AS name
          FROM nodes
         WHERE node_num = ?
        """,
        (node_num,),
